...
public class ConnectDatabase {
	...
	public static void main(String args[]) throws IOException, Exception {
		...
		final int aggregationTimeout = Integer.parseInt(p
				.getProperty("aggregation_sleepTime"));
		...
		final int serviceRequestTimeout = Integer.parseInt(p
				.getProperty("serviceRequest_db_timeout"));
		final int AGGREGATION_POOL_SIZE = Integer.parseInt(PropertiesUtil
				.getProperty("THREAD_POOL"));
		final int cache_POOL_SIZE = Integer.parseInt(PropertiesUtil
				.getProperty("CacheUpdateThread_POOL"));
		final int followUpThread_POOL_SIZE = Integer.parseInt(PropertiesUtil
		...
		// Dinesh Commented
		/*
		 * serviceRequestThread sr = new
		 * serviceRequestThread(connectionPool,serviceRequestTimeout);
		...
		ExecutorService followupmessageExecSvc = Executors
				.newFixedThreadPool(followUpThread_POOL_SIZE);
		for (int threadId = 0; threadId < followUpThread_POOL_SIZE; threadId++) {
			followupmessageExecSvc.execute(new FollowUpThread(
					followUpThread_POOL_SIZE, threadId, connectionPool,
					contactFollowUpTimeout));
		}
		...
		// Dinesh Commented
		/*
		 * ExecutorService cacheExecSvc =
		 * Executors.newFixedThreadPool(cache_POOL_SIZE); for (int threadId = 0;
		 * threadId < cache_POOL_SIZE; threadId++) { cacheExecSvc.execute(new
		 * CacheUpdateThread(cache_POOL_SIZE, threadId, connectionPool,
		 * cacheUpdateTimeout)); } cacheExecSvc.shutdown();
		 */
	}
}
