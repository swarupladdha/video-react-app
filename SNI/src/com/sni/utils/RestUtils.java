...
public class RestUtils {
	...
	/*public static String getOtpNotValidatedResponse(String serviceType, String functionType, JSONObject existingProfile,String session_id) {
		...
		prObject.put(OFDInterfaceKeys.otpvalidation, existingProfile.getString(OFDInterfaceKeys.otpvalidation));
		prObject.put(OFDInterfaceKeys.message, PropertiesUtil.getProperty("otp_notvalidated_message"));
		responseJsonObject.put(OFDInterfaceKeys.profileInfo, prObject);
	...
	/*public static String getEmailNotValidatedResponse(String serviceType,	String functionType, JSONObject existingProfile,String session_id) {
		JSONObject prObject = new JSONObject();
		JSONObject responseJsonObject = new JSONObject();
		prObject.put(OFDInterfaceKeys.emailvalidation, existingProfile.getString(OFDInterfaceKeys.emailvalidation));
		prObject.put(OFDInterfaceKeys.message, PropertiesUtil.getProperty("email_notvalidated_message"));
		responseJsonObject.put(OFDInterfaceKeys.profileInfo, prObject);
		String message = RestUtils.processSucess(serviceType, functionType, responseJsonObject,session_id);
		return message;
	}*/
	...
	// process success
		  ...
		  public static String processSucess(String serviceType, String functionType, JSONObject dataArray,String session_id) { 
		  ...
		  serviceJSON.put("session_id", session_id);
		  
		  statusJSON.put("code",PropertiesUtil.getProperty("statuscodesuccessvalue"));
		  statusJSON.put("message", PropertiesUtil.getProperty("statusmessagesuccessvalue")); 
	...
	/*public String formVerificationEmail(String validationLink) {
		...
		String text1 = OFDInterfaceKeys.welcomeToOFD;
		String text2 = OFDInterfaceKeys.clickOnLinkToActivate;
		String text3 = OFDInterfaceKeys.ifRecievedMailByMistake;
		String text4 = OFDInterfaceKeys.sincerely;
		String text5 = OFDInterfaceKeys.ofdTeam;
...
/*	public static String formQuery(String updateJob, String newvalue, String existingvalue, String tablefieldname) {
	...
	*/
	/*public String forgotPasswordEmail(String validationLink) {
		...
		String text1 = OFDInterfaceKeys.welcomeToOFD;
		String text2 = OFDInterfaceKeys.toResetPassword;
		String text3 = OFDInterfaceKeys.ifRecievedMailByMistake;
		String text4 = OFDInterfaceKeys.sincerely;
		String text5 = OFDInterfaceKeys.ofdTeam;
	...
	/*public static JSONObject checkUserIsValid(JSONObject existingProfile) {
			...
			profileInfo.put(OFDInterfaceKeys.plancode, existingProfile.getString(OFDInterfaceKeys.plancode));
			profileInfo.put(OFDInterfaceKeys.otpvalidation, existingProfile.getInt(OFDInterfaceKeys.otpvalidation));
			profileInfo.put(OFDInterfaceKeys.emailvalidation,existingProfile.getInt(OFDInterfaceKeys.emailvalidation));
			profileInfo.put(OFDInterfaceKeys.emailvalidation,existingProfile.getInt(OFDInterfaceKeys.emailvalidation));
			return profileInfo;
	}
	...
	public static int getRoleMandatory(JSONArray rol) {
			if(rol!=null){
				for (Object j : rol) {
					if (JSONObject.fromObject(j).getInt(OFDInterfaceKeys.mandatorycomplete) == 0) {
						return 0;
					}
				}
			}
	...
	/*public static String validateUser(String serviceType, String functionType, JSONObject profileInfo,String session_id) {
		...
		if ((profileInfo.getInt(OFDInterfaceKeys.otpvalidation)) == 0 ) {
			profileResponse =RestUtils.getOtpNotValidatedResponse(serviceType,functionType,profileInfo, session_id);
			return profileResponse;
		}
		if ((profileInfo.getInt(OFDInterfaceKeys.emailvalidation)) == 0) {
...
*/
	...
	public static JSONObject getJSONArrayFromDB(ResultSet res, String fieldname, String fieldtablename, JSONObject profileInfo) {
		...
		System.out.println(durationArray[1]);
		if(durationArray[1].contains("week") ||  durationArray[1].contains("Week")){
			System.out.println("=================================================");
			System.out.println(durationArray[0]);
			System.out.println(durationArray[1]);
		...
		if(durationArray[1].contains("month") || durationArray[1].contains("Month")){
...
/*		public static String ValidateRecords(JSONObject records,String start_no, String count) {
			...
			if (RestUtils.isEmpty(start_no) == false ||!( records.containsKey(OFDInterfaceKeys.start_no))) {
			...
			if (RestUtils.isEmpty(count) == false || !( records.containsKey(OFDInterfaceKeys.count))) {
		...
		/*public static String checkFeatureIsPermitted(String enabled, JSONObject permission) {
			...
			try {
				...
				if(permission.containsKey(OFDInterfaceKeys.AllAccessPermitted)){
					if(permission.getInt(OFDInterfaceKeys.AllAccessPermitted)==1){
						return response;
					}
					else{
						response = RestUtils.processError(PropertiesUtil.getProperty("invalid_permissions_code"),PropertiesUtil.getProperty("invalid_permissions_message"));
...
*/
		...
		public static java.sql.Date getJobDate(String string) {

			DateFormat format = new SimpleDateFormat("dd/MM/yyyy");
			Date sql;

			try {
				sql = format.parse(string);
				java.sql.Date dob = new java.sql.Date(sql.getDate());
				return dob;
			} catch (ParseException e) {
				e.printStackTrace();
			}		
		...
		public static String getDisplayAboutMe(String aboutMe) {
			String abtArray[] = aboutMe.split(" ");
			String displayAboutMe ="";
			for (int i = 0; i < abtArray.length; i++) {
				if (abtArray[i].length()>10) {
					displayAboutMe+=abtArray[i].substring(0,7)+"... ";
				}
				else{
					displayAboutMe+=abtArray[i]+" ";
				}
				
			}
			return displayAboutMe.trim();
		}
