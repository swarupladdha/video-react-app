...
import com.opentok.Archive.Status;

import net.sf.json.JSONObject;

public class BackUpPlan {

	static int apiKey = Integer.parseInt(PropertiesUtil.getProperty("apiKey")); // YOUR API KEY
	static String apiSecret = PropertiesUtil.getProperty("apiSecret");
	public static void main(String[] args) {
		Scanner s = new Scanner(System.in);
		System.out.println("Please enter session id :");
		String sid = s.next();
		BackUpPlan p = new BackUpPlan();
		String res = p.getArchiveUrl(sid);

		if (res != null ) {
			System.out.println("The url is :");
			System.out.println(res);
			Test t = new Test();
//			res="https://s3.amazonaws.com/tokbox.com.archive2/46128652/b86e9db4-330f-4ab9-8b1f-2906e2c6f055/archive.mp4?AWSAccessKeyId=AKIAI6LQCPIXYVWCQV6Q&Expires=1537971910&Signature=iQHyut0Y6pR3USG5wplMSfyBggo%3D";
			System.out.println(res);
			t.connectAndGet(res);
		}
		else {
			System.out.println("achive url recieved is null");
		}
		s.close();
	}
	
	public String getArchiveUrl(String sessionId) {
		String eduResponse="";
		try{
			...
			System.out.println("--------------------------------------------------------------");
			System.out.println(RestUtils.isEmpty(sessionId) == false);
			...
			for (int i = 0; i < archives.size(); i++) {
				...
				System.out.println(sessionId);
				System.out.println(" match is "+archives.get(i).getSessionId().equals(sessionId));
				System.out.println("--------------------------------");
				if(archives.get(i).getSessionId().equals(sessionId)){
					archiveId = archives.get(i).getId();
					archiveUrl= archives.get(i).getUrl();
					archiveDuration = archives.get(i).getDuration();
					archiveSize= archives.get(i).getSize();
					archiveName= archives.get(i).getName();
					archiveCreatedAt= archives.get(i).getCreatedAt();
					archiveStatus= archives.get(i).getStatus();
					...
					System.out.println("archieve exists getDuration : "+archives.get(i).getDuration());
					System.out.println("archieve exists getSize : "+archives.get(i).getSize());
					System.out.println("archieve exists getCreatedAt : "+archives.get(i).getCreatedAt());
			...
			responsedata.put("archiveurl", archiveUrl);
			responsedata.put("archiveid", archiveId);
			...
			responsedata.put("archiveStatus", archiveStatus);
		//	eduResponse = RestUtils.processSucess(serviceType,functionType,responsedata,"");
		//	return eduResponse;
			if(RestUtils.isEmpty(archiveUrl)) {
				return archiveUrl;
			}
			else {
				return null;
