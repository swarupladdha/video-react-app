...
public class StartSMSCountryReport implements Runnable {
	...
	static {
		...
		try {
			...
			System.setProperty("Hibernate-Url", StartSMSCountryReport.class
					.getResource("/Hibernate.cfg.xml").toString());
			InputStream in = StartSMSCountryReport.class
			...
			InputStream login = StartSMSCountryReport.class
	...
	public static void getInit() {
		try {
			...
			String ipaddr = prop.getProperty("smscountry_host");
			String port = prop.getProperty("smscountry_port");
			int portaddr = Integer.parseInt(port);
			String usrname = prop.getProperty("smscountry_username");
			String password = prop.getProperty("smscountry_password");
				...
				public void onAcceptDeliverSm(DeliverSm deliverSm)
					...
					if (MessageType.SMSC_DEL_RECEIPT.containedIn(deliverSm
						...
						try {
							...
							e.printStackTrace();
							logger.error(
									"Exception occured in receiving sms report receipt.",
									e);
						}
			...
			logger.info(
					"Failed connect and bind to host to SmsCountry report connection.",
					e);
			e.printStackTrace();
			...
			logger.info("Failed connect and bind to host to SmsCountry report connection. -- "
					+ printString);
	...
	public void run() {
		while (true) {
			...
			try {
				...
				if (!sessionrec.isReceivable()
						|| sessionrec.equals(SessionState.CLOSED)
				...
				logger.info("Exception occured while trying rebind inside thread -- "
						+ printString);
	...
	public static void main(String args[]) {
		try {
			StartSMSCountryReport smscountryobj = new StartSMSCountryReport();
			...
			System.err.println("Exception in SmsCountry main");
			logger.info("Exception in SMS Country main.", e);
			e.printStackTrace();
		}
	}
