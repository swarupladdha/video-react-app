...
public class VinrMessagesInTable {
    ...
    public ResultSet readNewMessages(int threadId) {
        ...
        try {
            ...
            String QueryString = new StringBuffer("select MsgId, MsgType, Address, Message, Version, Provider, Attachment, Date, CustomData from MessagesInTable where (").
                                           append("Date <= sysdate() and ((").
                                           append("MsgId % ").
                                           append(THREAD_POOL_SIZE).
                                           append(")").
                                           append(" = ").
                                           append(threadId).
                                           append("))").toString();	                	
    ...
    public int deleteMessage (int msgId) {
	    ...
	    stmt = connection.createStatement();
	    String QueryString = "delete from MessagesInTable where MsgId = \'" + msgId + "\'";
    ...
    public int touchMessage (int msgId , long postpone_secs) {
	    ...
	    stmt = connection.createStatement();
	    String QueryString = "update MessagesInTable set date = DATE_ADD(sysdate(), INTERVAL " + postpone_secs + " second) where MsgId = \'" + msgId + "\'";
	    //System.out.println(QueryString);
