...
/** This will parse the XML retrieved from database , compares the 
  * the tagName, retrieves and returns the Text - Content of each 
  * tag(Node).
  *
  * @author Sushma P
  */
public class VinrMessageParser {
     ...
     * converts it to the required inputStream. Using the DocumentBuilderFactory
     * we create a new instance of DocumentBuilder and parses the inputStream to
     * to the DocumentBuilder instance to obtain DOM Document instances from the
     * XML document.
     * 
     * @param input String : Is a concatenation of 'Address' and 'Message' columns
     * of 'messagesintable'
     */
     ...
     * TextContent of the respective 'Node' like the <tolist>, <from>,
     * <subject>, etc.. nodes 
     */
    ...
    /** This routine returns the text-content of '<tolist>' Node
     * The <tolist> contains the address column of messagesintable
     * (email-id when msgType=0, mobileNumber when msgType=1, email-id 
     * and mobileNumber when msgType=2, system-id, user-id, password, 
     * etc. depending on the provider)
     *
     * @param input String : Node whose text-content is to be extracted.
     */
    ...
    private void processCcList(Node ccNode) {
	    ...
	    int ccNodeLength = ccNodes.getLength();
	 	...
	 	if (tagName.equalsIgnoreCase("name") == true) {
            ...
            if (((cctag.equalsIgnoreCase("email")) && (cctag1.equalsIgnoreCase("number"))) || ((cctag.equalsIgnoreCase("number")) && (cctag1.equalsIgnoreCase("email")))) {
               String ccEmailId = "";
               String ccMobNo = "";
            ...
            //To return the textContent of either 'email' or 'sms' Node depending on the msgType 
	    if (ccNodeLength == 2) {
                Node nameNode = ccNodes.item(0);
	 	String tagName = nameNode.getNodeName();
	        if (tagName.equalsIgnoreCase("name") == true) {
     ...
     *
     * @param input String : shortText Node 
     */
	...
	int totalChildren = fromNodes.getLength();
		...
		String secondFromTagName = secondFromTag.getNodeName();
                ...
                if (((firstFromTagName.equalsIgnoreCase("email")) && (secondFromTagName.equalsIgnoreCase("number"))) || ((firstFromTagName.equalsIgnoreCase("number")) && (secondFromTagName.equalsIgnoreCase("email")))) {
                    String fromEmailId = "";
