...
public class StaticUtils {
	...
	public static void initializeTelephoneCodes() {
		...
		List<PhoneCodesList> phcodelist = (List<PhoneCodesList>) PhoneCodesList
				.getListofPhoneCodes();
	...
	public static String FormatLandlineNumber(String landlineNo,
			String ivrnumber) {
		...
		if (landlineNo.length() > 8) {
				...
				if (landlineNo.startsWith(cc)) {
					...
					for (String code : areaCodeList) {
						...
						if (landlineNum.startsWith(code)) {
							...
							landlineNum = landlineNum.substring(code.length(),
									landlineNum.length());
			...
			if (StaticUtils.isEmptyOrNull(countryCode)) {
				...
				if (StaticUtils.isEmptyOrNull(areaCode)) {
					...
					tempIvrNum = ivrnumber.substring(countryCode.length(),
							ivrnumber.length());

					List<String> areaCodeListivr = telephonecodes
							.get(countryCode);
	...
	public static String validateMobile(String callerId, String ivrNumber)
			throws Exception {

		String trimmedMob = StaticUtils.TrimPhoneNumber(callerId);

		String formattedMobNo = StaticUtils.FormatMobileNumber(trimmedMob,
				ivrNumber);
	...
	public static String validateLandline(String callerId, String ivrnumber)
			throws Exception {
		...
		formattedLLNo = StaticUtils
				.FormatLandlineNumber(trimmedLLNo, ivrnumber);
	...
	public static Response processUrlOrTextMultiList(String displayList,
			int playspeed, int timeout) {
	...
	public static Response sendNotRegGrupzResp(IvrGroupzBaseMapping ivrnummap,
			ContextMapping cm) {
	...
	public static Response playUrlOrTextMessage(String text, String audio,
			int playspeed, ContextMapping co, boolean multiLangFlag) {
		Response kkResponse = new Response();

		if (multiLangFlag && StaticUtils.isEmptyOrNull(audio) == false) {
			audio = StaticUtils.getSelectedLangUrl(audio, co, multiLangFlag);
	...
	public static Response processUrlOrTextMessage(String audio, String text,
			String defualttext, String defualtUrl, int playspeed,
			boolean multiLangFlag, ContextMapping co) {
	...
	public static Response senderrorResp(String callSessionId,
			String ivrNumber, ContextMapping co) {
	...
	public static ArrayList<String> numberUrlsDisplayString(String numberUrls,
			ArrayList<String> nameslist, boolean regionalLanguage) {
		...
		if (numberUrls != null && numberUrls.isEmpty() == false) {
			...
			if (regionalLanguage) {
				...
				for (int i = 0; i < nameslist.size(); i++) {
					...
					String groupzName = nameslist.get(i);
	...
	public static Response createMultiGroupzData(IvrGroupzBaseMapping ivrnummap,
			HashMap<String, String> groupzcodeNamMap,
			List<String> ivrgrpzcodeList,
			HashMap<String, String> groupzinfoMap, int playspeed, int timeout,
			ContextMapping cm) {
		...
		if (ivrnummap != null) {
			...
			if (multilangFlag) {
				...
				if (StaticUtils.isEmptyOrNull(multiGrpzWelcomeUrl) == false) {
					multiGrpzWelcomeUrl = StaticUtils.getSelectedLangUrl(
							multiGrpzWelcomeUrl, cm, multilangFlag);
				}

				if (StaticUtils.isEmptyOrNull(selectionmultigrpzhangupurl) == false) {
					selectionmultigrpzhangupurl = StaticUtils
							.getSelectedLangUrl(selectionmultigrpzhangupurl,
									cm, multilangFlag);
				}
				...
				if (StaticUtils.isEmptyOrNull(nummberListUrls) == false) {
					...
					selectedUrllist = numberurlsListObj
							.getJSONObject(languageSelected);
	...
	public static String createMemberlistString(
			String callSessionId, List<GroupzMemberInfo> memberList,
			ContextMapping cm, String ivrNumber) {
		...
		memberSelectionEndArray = StaticUtils
				.createJSONdataArray(selectionendNote);

		memberWelcomeArray.addAll(multiMemberDataArray);
		memberWelcomeArray.addAll(memberSelectionEndArray);

		if (multiGroupzWelcomenotes != null
				&& multiGroupzWelcomenotes.isEmpty() == false) {
	...
	public static String Createrespobject(String responseString,
			String statuscode) {
	...
	public static String getGroupzInfoXML(String countryCode, String number,
			String stateCode, String request, String functiontype) {
		...
		try {
			...
			logger.info(
					"Exception occured while sending request for groupzinfo List for mobile number.",
					e);
			e.printStackTrace();
	...
	public static String getMemberInfoXML(String countryCode, String number,
			String stateCode, String request, String functiontype, String grpzid) {

		String responsexmlString = null;
		try {
			String servicetype = prop.getProperty("servicetype");

			String urltoinvoke = prop.getProperty("memberListUrl");
	...
	public static String ConnectAndGetResponse(String urlString,
			String xmlString) {
		...
		try {
			...
			if (statusCode.equals("0")) {
				...
				if (obj instanceof JSONArray) {
					...
					JSONObject jsngrpzlistObj = jores
							.getJSONObject("memberslist");
	...
	public static String getSelectedLangUrl(String urlsString, ContextMapping co,
			boolean multiLangFlag) {
		String selectedUrl = null;
		if (multiLangFlag && StaticUtils.isEmptyOrNull(urlsString) == false) {

			String languageSelected = null;

			if (co != null) {
				languageSelected = co.getlanguageSelected();
			}
			...
			JSONObject langObj = (JSONObject) JSONSerializer.toJSON(urlsString);
	...
	public static boolean getJSONStatusCode(String responseJSONString,String grpzCode) {
		...
		try {
			...
			JSONObject jsnrespObj = (JSONObject) JSONSerializer
					.toJSON(responseJSONString);
			
			JSONObject jsonObj = jsnrespObj
					.getJSONObject("json");
	...
	public static ArrayList<String> getGrpzMobileValidationList(
		...
		try {
			...
			grpzInfoxmlString = StaticUtils.getGroupzInfoXML(countryCode,
					mobileNumber, null, "mobile", functionType);
			...
			logger.info(" exception in get mobile validation list :"
	...
	public static ArrayList<String> getGrpzLandlineValidationList(
			String formattedNumber, String ivrNumber) {
		...
		try {
			...
			grpzInfoxmlString = StaticUtils.getGroupzInfoXML(countryCode,
					number, stateCode, "landline", functionType);

			System.out.println("xmllandline :" + grpzInfoxmlString);

			if (StaticUtils.isEmptyOrNull(grpzInfoxmlString)) {

				logger.info(" grpzIfoXMl is empty for landline :" + "number : "
						+ formattedNumber);
			...
			logger.info(" exception in get landline validation status :"
					+ "number : " + formattedNumber + "exception data :" + e);
			groupzInfodetailList.add(statusFlag);
			groupzInfodetailList.add(grpzInfoxmlString);
	...
	public static ArrayList<String> getMemberMobileValidationList(
			String formattedNumber, String grpzID) {
		...
		try {
			...
			membXmlInfo = StaticUtils.getMemberInfoXML(countryCode,
					mobileNumber, null, "mobile", functionType, grpzID);

			if (StaticUtils.isEmptyOrNull(membXmlInfo)) {

				logger.info(" membXmlInfo for mobile is empty :" + "number : "
						+ formattedNumber);
			...
			logger.info(" exception in get mobile member list  :" + "number : "
					+ formattedNumber + "exception data :" + e);
	...
	public static ArrayList<String> getMemberLandLineValidationList(
			String formattedNumber, String grpzID) {
		...
		try {
			...
			membXmlInfo = StaticUtils.getMemberInfoXML(countryCode,
					mobileNumber, statecode, "landline", functionType, grpzID);

			if (StaticUtils.isEmptyOrNull(membXmlInfo)) {

				logger.info(" membXmlInfo for landline is empty :"
			...
			logger.info(" exception in get landline member list  :"
					+ "number : " + formattedNumber + "exception data :" + e);
	...
	public static Response displayTextOrURLforSelection(String text,
			String audioUrl, int playspeed, int timeout,ContextMapping contxt) {
	...
	public static String createUrlListJSONString(String url, String lan,
			String jsonString,ContextMapping cm) {
	...
	public static void main(String args[]) {
		...
		String xml = StaticUtils
				.ConnectAndGetResponse(
						"http://code.groupz.in:8080/RestAPI/SelectGroupzServlet?",
						"<xml><request><functiontype>2001</functiontype><mobile><countrycode>91</countrycode><mobilenumber>9986067657</mobilenumber></mobile><modulecode>8</modulecode><servicetype>21</servicetype></request></xml>");
