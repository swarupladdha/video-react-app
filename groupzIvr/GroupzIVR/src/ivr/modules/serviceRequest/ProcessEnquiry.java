...
public class ProcessEnquiry {
		...
		public String processNewCall(String callerId, String ivrNumber, String callSessionId)
		{
			...
			try
			{
						...
						&& StaticUtils.isEmptyOrNull(callSessionId) == false)
				{
					...
					{
						responseXMl = ProcessEnquiry.processDetailedNewCall(callerId, ivrNumber, callSessionId);
		...
		{
			System.out.println("++++++++++++ Insde the function call processDetailedNewCall inside Enquiry ++++++++++");
			Response kkResponse = new Response();
					...
					if (size > 1)
					{
						...
						if (regisgrpzcodeList != null && regisgrpzcodeList.size() > 0)
						{
							...
							else
							{
								...
								System.out.println("1.------------->>"+enquiryflag);
					...
					if (globalsingleregstrdgrpz)
					{
						...
						if (memberList != null && memberList.size() > 0)
						{
							...
							if (memblistSize > 1)
							{
								...
								System.out.println("2.-------------->>"+enquiryflag);
							...
							else if (memblistSize == 1)
							{
								...
								System.out.println("3.----------------->>"+enquiryflag);
					...
					if (groupzList != null && groupzList.isEmpty() == false)
					{
						...
						if (ivrgrpzcodeList != null	&& ivrgrpzcodeList.isEmpty() == false)
						{
							if (ivrgrpzcodeList.size() > 1)
							{
								...
								System.out.println("4.---------->>"+enquiryflag);
							...
							else if (ivrgrpzcodeList.size() == 1)
							{
								...
								if (memberList != null && memberList.size() > 0)
								{
									...
									else if (memblistSize == 1)
									{
										...
										cm.setEnquiryflag(enquiryflag);
										cm.save();
										System.out.println("5.-------------------->"+enquiryflag);
						...
						if (selectionListObj.containsKey(data))
						{
							...
							{	
								...
								if (StaticUtils.isEmptyOrNull(selectedOption) == false)
								{
									...
									String responseXMl = ProcessEnquiry.processDetailedNewCall(formattednumber, ivrNumber, 
											callSessionId);
							...
							else if (multigrpzFalg == false && multimembFalg == false && multiLanguageFlag == false)
							{
								...
								if (statusFlag)
								{
									...
									if (enquiryFlag)
									{
										System.out.println("We are processing your enquiry Request !");
									...
									{
										loggerServ.info("Problem in getting enquiry details for number and callSessionID :  "
